...
const AccordionContact: React.FC<AccordionProps> = ({ title, description }) => {
      ...
      >
        {isOpen && (
          <div className=" ml-[50px] h-fit min-h-[50px] mb-[10px] w-fit">
            <p className="text-[#888888] border-l-1 border-[#0073B4] text-[20px] max-xl:text-[16px] max-md:text-[13px] pl-[20px] align-middle pt-1 w-fit">{description} <Link href='/Contact' className='underline'>Contact Us</Link> (Button To Contact Us Page)</p>
          </div>
        )}
      </div>
    </div>
