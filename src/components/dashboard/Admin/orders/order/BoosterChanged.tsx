...
import { getOrderDetails, sendBackToOrdeList, sendMessage } from 'src/lib/actions/admin.dashboard.actions';
...
const BoosterChanged = ({order}: {order: string}) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const res = await getOrderDetails(JSON.parse(JSON.stringify(order)), 'change_booster');
        if (res) {
      ...
      {orderDetails ? (
        <div className='flex flex-col w-full relative'>
          ...
          <div className='w-full flex justify-between'>
            <div className='text-center inline-block w-[20%]'>
            ...
            <div className='inline-block w-[20%] text-center'>
              ...
              <span className='text-medium text-gray-600'>Booster Changed Date</span>
            </div>
          </div>
        </div>
      ) : (
