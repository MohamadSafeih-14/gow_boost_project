...
const DroppedOrder = ({order}: {order: string}) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const res = await getOrderDetails(JSON.parse(JSON.stringify(order)), 'drop_order');
        if (res) {
        ...
        <div className='flex flex-col w-full relative'>
          ...
          <div className='w-full flex justify-between'>
            ...
            <div className='text-right inline-block w-[20%]'>
              ...
              <span className='text-medium text-gray-600'>Dropped Date</span>
            </div>
          </div>
        </div>
      ) : (
