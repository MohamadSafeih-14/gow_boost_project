...
const FinishedOrder = ({order}: {order: string}) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const res = await getOrderDetails(JSON.parse(JSON.stringify(order)), "booster_finished");
        if (res) {
  ...
  const handleCompleteOrder = async () => {
    const res = await completeOrder(orderDetails.orderId, "booster_finished");
    if(res.status === 200) {
      toast.success(res.message)
      router.refresh();
    } else {
