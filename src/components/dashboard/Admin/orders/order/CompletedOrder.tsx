...
import { deleteOrder, getOrderDetails, sendBackToOrdeList, sendMessage } from 'src/lib/actions/admin.dashboard.actions';
...
const CompletedOrder = ({order}: {order: string}) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const res = await getOrderDetails(JSON.parse(JSON.stringify(order)), 'completed_order');
        if (res.status === 200) {
  ...
  const handleDeleteOrder = async () => {
    const res = await deleteOrder(orderDetails.orderId);
    if(res.status === 200) {
      toast.success(res.message)
    } else {
        toast.error(res.message)
      ...
      {orderDetails ? (
        <div className='flex flex-col w-full relative'>
          ...
          <div className='w-full flex justify-between'>
            ...
            <Button color="danger" className='mt-5 w-[20%] text-lg' onClick={handleDeleteOrder}>Delete Order</Button>
            <div className='text-right inline-block w-[20%]'>
              ...
              <span className='text-medium text-gray-600'>Request Date</span>
            </div>
          </div>
        </div>
      ) : (
